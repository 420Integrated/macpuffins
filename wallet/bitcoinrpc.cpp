...
static inline unsigned short GetDefaultRPCPort() { return GetBoolArg("-testnet", false) ? 11784 : 1784; }
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error("stop\n"
                            "stop macpuffins server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "macpuffins server stopping";
}
...
{ //  name                         function                    safemd  unlocked
  //  ------------------------     -----------------------     ------  --------
    ...
    { "udtomacpuffinsaddress",     &udtomacpuffinsaddress,     true,   false },
...
string HTTPPost(const string& strMsg, const map<string, string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: macpuffins-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("macpuffins-rpclist");
...
void ThreadRPCServer2(void* /*parg*/)
{
    ...
    if ((rpcPassword == "") || (rpcUser == rpcPassword)) {
        ...
        string strWhatAmI = "To use macpuffinsd";
        ...
        uiInterface.ThreadSafeMessageBox(
            strprintf(
                _("%s, you must set a rpcpassword in the configuration file:\n %s\n"
                  ...
                  "rpcuser=macpuffinsrpc\n"
                  "rpcpassword=%s\n"
                  "(you do not need to remember this password)\n"
                  "The username and password MUST NOT be the same.\n"
                  "If the file does not exist, create it with owner-readable-only file permissions.\n"
                  "It is also recommended to set alertnotify so you are notified of problems;\n"
                  "for example: alertnotify=echo %%s | mail -s \"macpuffins alert\" admin@foo.com\n"),
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("macpuffins-rpchand");
...
GetNTP1RecipientsVector(const Object& sendTo, boost::shared_ptr<NTP1Wallet> ntp1wallet)
{
    ...
    for (const json_spirit::Pair& s : sendTo) {
        ...
        if (s.value_.type() == json_spirit::Value_type::obj_type) {
            ...
            res.tokenId = NTP1SendTxData::PFN_TOKEN_ID;
        }
